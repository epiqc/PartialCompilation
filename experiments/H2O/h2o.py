...
file_name = datetime.today().strftime('%h%d_h2o_slice_%s' % sys.argv[1])
...
h2o_circuit = uccsd.get_uccsd_circuit('H2O')
h2o_circuit = util.circuitutil.optimize_circuit(h2o_circuit)
coupling_list = util.circuitutil.get_nearest_neighbor_coupling_list(2, 5)
h2o_circuit = util.circuitutil.optimize_circuit(h2o_circuit, coupling_list)
...
gates = h2o_circuit.data
width = h2o_circuit.width()
...
while len(gates) > 0:
    ...
    slice_circuits = [deepcopy(h2o_circuit) for _ in range(len(blocking.blocks))]
