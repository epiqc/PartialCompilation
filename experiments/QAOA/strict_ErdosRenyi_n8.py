...
graph_N = 8
num_slices = [7,13,19,25,31,37,43,49] # for p = 1,2,3,...
sbatch_id = int(sys.argv[1])
(graph_p, slice_id) = find_p(num_slices, sbatch_id)

print("Experiment Strict ErdosRenyi N=%d p=%d slice=%d" % (graph_N, graph_p, slice_id))
data_path = config.DATA_PATH
file_name = datetime.today().strftime('%h%d_strictErdosRenyiN%dp%d_slice_%s' % (graph_N, graph_p, slice_id))
