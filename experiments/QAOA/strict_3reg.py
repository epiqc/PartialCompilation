...
# Parameters for the 3-regular graph
def find_p(num_s, i):
    res = 0
    old_res = 0
...
graph_N = 6
num_slices = [7,10,13,16,19,22,25,28,31,34] # for p = 1,2,3,...
num_slices = num_slices[:8]
sbatch_id = int(sys.argv[1])
(graph_p, slice_id) = find_p(num_slices, sbatch_id)

print("Experiment Strict 3Reg N=%d p=%d slice=%d" % (graph_N, graph_p, slice_id))
data_path = config.DATA_PATH
file_name = datetime.today().strftime('%h%d_strict3regN%dp%d_slice_%s' % (graph_N, graph_p, slice_id))
