...
file_name = datetime.today().strftime('%h%d_nah_slice_%s' % sys.argv[1])
...
gates = nah_circuit.data
width = nah_circuit.width()
...
while len(gates) > 0:
    ...
    slice_circuits = [deepcopy(nah_circuit) for _ in range(len(blocking.blocks))]
