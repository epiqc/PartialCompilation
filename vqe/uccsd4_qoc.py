...
from uccsd4_unitary import unitary as uccsd4_unitary
...
THETA = 2.0
U = uccsd4_unitary(THETA) 
# print("target unitary", U, U.shape)

# Define controls
# 
Q_x1 = kron_many(Q_x,Q_I, Q_I, Q_I)
Q_y1 = kron_many(Q_y,Q_I, Q_I, Q_I)
Q_z1 = kron_many(Q_z,Q_I, Q_I, Q_I)
Q_x2 = kron_many(Q_I,Q_x, Q_I, Q_I)
Q_y2 = kron_many(Q_I,Q_y, Q_I, Q_I)
Q_z2 = kron_many(Q_I,Q_z, Q_I, Q_I)
Q_x3 = kron_many(Q_I, Q_I, Q_x, Q_I)
Q_x4 = kron_many(Q_I, Q_I, Q_I, Q_x)
Q_z4 = kron_many(Q_I, Q_I, Q_I, Q_z)

# Hops = [Q_x1, Q_x2, Q_z2]
# For 4 qubits we expect the following:
Hops = [Q_x1, Q_x2, Q_x3, Q_x4, Q_z4]
# ops_max_amp = [2*np.pi*0.5, 2*np.pi*0.5, 2*np.pi*0.5]
ops_max_amp = [np.pi for _ in range(5)]
Hnames =['x1', 'x2', 'x3', 'x4', 'z4']
